...
主题内置两套配色：默认的暖调「墨色」（ink）和冷调淡蓝「清新」（fresh）。点顶栏的调色板按钮即可切换并记住选择；也可以把 `site-config.ts` 里的 `site.palette` 改成 `'fresh'`，作为新访客看到的默认配色。两套配色的变量都定义在 `src/assets/styles/tokens.css`，直接改数值就能调出你自己的色板。
